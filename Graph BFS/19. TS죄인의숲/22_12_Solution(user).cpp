...
struct Pair {
    LL first, second;  // 121bit를 61bit, 60bit로 나누어 저장
...
Pair codeTable[LM][LM];          // D*D단위로 64bit정수 2개로 묶어 저장
int detectResult[DLM][DLM];      // useDetector() 결과
int dr[] = { -1, 0, 1, 0 };      // clockwise : 12시, 3시, 6시, 9시
int dc[] = { 0, 1, 0, -1 };
...
Pair getCode(int r, int c) {     // 숲을 bitset으로 : (r, c)에 죄인이 있다고 할 때,
...
void init(int N, int D, int mForest[LM][LM], int K) {
    ...
    for (int i = 0; i < N; ++i)for (int j = 0; j < N; ++j) {    // (i, j)를 중심으로 D*D영역을 64bit 2개의 정수로
...
Result findCriminal() {
    ...
    while (answerFixed == false && fr < re) {
        ...
        while (fr < ed) {
            Data t = que[fr++];
            for (int i = 0; i < 4; ++i, t.d = (t.d + 1) & 3) { // t.d = (t.d + 1) % 4 : 시계방향으로 90도 회전
                // t로부터 현재 방향으로 한칸 이동하기
